...
{
    public partial class ordinaToGilardiFilatelia : System.Web.UI.Page
    {
       protected void Page_Load(object sender, EventArgs e)
        {
            ...
            Boolean esitoAdmin = MailList.OffertaGilardiFilatelia(riferimento, subject, email, bodyMail, oraSpedizione);
            if (esitoAdmin == false)
            {
                if (sendMeToCopy)
                {
                    Boolean esitoGuest = MailList.OffertaGilardiFilateliaToGuest(riferimento, subject, email, bodyMail);
                    if (esitoGuest == false)
                    {
